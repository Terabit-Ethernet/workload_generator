...
def poissonFlowGenerator(packet_size, header_size, mean_size, num_flows, num_hosts, bandwidth, load, filename, smooth):
    pq = []
    nv_bytes = EmpiricalRandomVariable(filename, smooth,packet_size,header_size)

    mean_flow_size = nv_bytes.mean_flow_size
    if(mean_size > 0):
        mean_flow_size = mean_size * 1000
...
def poissonFlowGeneratorMaxLimits(packet_size, header_size, mean_size, num_flows, num_hosts, bandwidth, load, filename, smooth,fcnt):
    pq = []
    nv_bytes = EmpiricalRandomVariable(filename, smooth,packet_size,header_size)
...
def poissonFlowGeneratorMaxLimitsRandDest(packet_size, header_size, mean_size, num_flows, num_hosts, bandwidth, load, filename, smooth,fcnt):
    pq = []
    nv_bytes = EmpiricalRandomVariable(filename, smooth,packet_size,header_size)
    ...
    for i in range(num_hosts):
        first_flow_time = 1.0 + nv_intarr.value()
        heapq.heappush(pq,[first_flow_time, i, nv_bytes, nv_intarr])
    ...
    while len(flows) < num_flows:
        ...
        if( (src_host_flow_cnt[src] < fcnt) and (dst_host_flow_cnt[dst] < fcnt) ):
            next_time = time + nv_intarr.value()
            flows.append([time, src, dst, flow_id, size])
            heapq.heappush(pq,[next_time, src, nv_bytes, nv_intarr])
            src_host_flow_cnt[src] += 1
            dst_host_flow_cnt[dst] += 1
        elif ((src_host_flow_cnt[src] < fcnt)):
            heapq.heappush(pq,[time, src, nv_bytes, nv_intarr])
        else:
            pass            
...
def poissonSemiPermutationFlowGeneratorMaxLimits(packet_size, header_size, mean_size, num_flows, num_hosts, bandwidth, load, filename, smooth,fcnt,alpha,iat):
    ...
    if(iat == 0):
        ...
        assert(mean_flow_size > 0) #only makes senses for constant sized flows
        nv_intarr_permutation = ConstantVariable(1.0/lambda_per_permutation_host)
    ...
    if(iat == 0):
        ...
        assert(mean_flow_size > 0)
        nv_intarr_all_to_all = ConstantVariable(1.0/lambda_per_all_to_all_host)
...
def poissonSemiPermutationFlowGeneratorMaxLimitsOversubscription(packet_size, header_size, mean_size, num_flows, num_hosts, bandwidth, load, filename, smooth,fcnt,alpha,iat,ov, num_hosts_per_tor):

    assert(ov >= 1)
    assert(num_hosts_per_tor >= 1)
    pq = []
    nv_bytes = EmpiricalRandomVariable(filename, smooth,packet_size,header_size)
    ...
    lambda_per_permutation_host_ov = lambda_per_permutation_host / ov
    lambda_per_all_to_all_host_ov = lambda_per_all_to_all_host / ov

    if(iat == 0):
        nv_intarr_permutation = ExponentialRandomVariable(1.0 / lambda_per_permutation_host)
        nv_intarr_permutation_ov = ExponentialRandomVariable(1.0 / lambda_per_permutation_host_ov)
    else:
        assert(mean_flow_size > 0) #only makes senses for constant sized flows
        nv_intarr_permutation = ConstantVariable(1.0/lambda_per_permutation_host)
        nv_intarr_permutation_ov = ConstantVariable(1.0/lambda_per_permutation_host_ov)
    ...
    for src in perm_srcs:
        ...
        perm_dsts.add(temp_dst)
        assert(src != temp_dst)
        if(int(src/num_hosts_per_tor) == int(temp_dst/num_hosts_per_tor)):
            first_flow_time = 1.0 + nv_intarr_permutation.value()
            heapq.heappush(pq,[first_flow_time, src, temp_dst, nv_bytes, nv_intarr_permutation])
        else:
            first_flow_time = 1.0 + nv_intarr_permutation_ov.value()
            heapq.heappush(pq,[first_flow_time, src, temp_dst, nv_bytes, nv_intarr_permutation_ov])

    if(iat == 0):
        nv_intarr_all_to_all = ExponentialRandomVariable(1.0 / lambda_per_all_to_all_host)
        nv_intarr_all_to_all_ov = ExponentialRandomVariable(1.0 / lambda_per_all_to_all_host_ov)
    else:
        assert(mean_flow_size > 0)
        nv_intarr_all_to_all = ConstantVariable(1.0/lambda_per_all_to_all_host)
        nv_intarr_all_to_all_ov = ConstantVariable(1.0/lambda_per_all_to_all_host_ov)
    ...
    for i in range(num_hosts):
        ...
        for j in range(num_hosts):
            if(j in perm_dsts):
                continue
            if i != j:
                if(int(i/num_hosts_per_tor) == int(j/num_hosts_per_tor)):
                    first_flow_time = 1.0 + nv_intarr_all_to_all.value()
                    heapq.heappush(pq,[first_flow_time, i, j, nv_bytes, nv_intarr_all_to_all])
                else:
                    first_flow_time = 1.0 + nv_intarr_all_to_all_ov.value()
                    heapq.heappush(pq,[first_flow_time, i, j, nv_bytes, nv_intarr_all_to_all_ov])
...
def poissonIncastGenerator(packet_size, header_size, mean_size, num_flows, num_hosts, bandwidth, load, filename, smooth,fcnt,alpha,iat):
    pq = []
    nv_bytes = EmpiricalRandomVariable(filename, smooth,packet_size,header_size)
    ...
    print(mean_flow_size)
    lmda = bandwidth * load / (mean_flow_size * 8.0 / (packet_size - header_size) * packet_size)
    lmda_per_host = lmda / 8.0 #Each end host will send to the incast receivers in other leaf switches

    if(iat == 0):
        nv_intarr = ExponentialRandomVariable(1.0 / lmda_per_host)
    else:
        assert(mean_flow_size > 0) #only makes senses for constant sized flows
        nv_intarr = ConstantVariable(1.0/lmda_per_host)

    srcs = set()
    dsts = {0,16,32,48,64,80,96,112,128}

    for i in range(num_hosts):
        srcs.add(i)

    assert(len(srcs) == num_hosts)
    ...
    for src in srcs:
        for dst in dsts:
            ...
            assert(src != dst)
            first_flow_time = 1.0 + nv_intarr.value()
            heapq.heappush(pq,[first_flow_time, src, dst, nv_bytes, nv_intarr])
...
def dcqcnIncastGenerator(packet_size, header_size, num_hosts_per_leaf, num_leaf, rtt, size, fcnt, incast_degree):
    ...
    avail_dst_set = set([16*i for i in range(num_leaf)])
    flows = []
    arrival_time = 1.0
    flowid = 0
...
def dcqcnIncastSemiPermGenerator(packet_size, header_size, num_hosts_per_leaf, num_leaf, rtt, size, fcnt, incast_degree, filename, smooth, alpha, load, mean_size, num_flows,bandwidth):
    num_hosts = num_hosts_per_leaf * num_leaf
    src_host_flow_cnt = [0] * num_hosts
    dst_host_flow_cnt = [0] * num_hosts
    avail_src_set = set(range(num_hosts))
    ...
    while(len(avail_src_set) > 0 and len(avail_dst_set) > 0 and incast_cnt > 0):
        arrival_time += rtt
        for i in range(num_leaf):
            ...
            incast_cnt -= 1
                # if(src_host_flow_cnt[src] >= fcnt-5):
                #     avail_src_set.remove(src)
            # if(dst_host_flow_cnt[dst] >= fcnt-incast_degree-5):
            #     avail_dst_set.remove(dst) 
    ...
    flows_perm = []
    while len(flows_perm) < num_flows:
        element = heapq.heappop(pq)
        flow_id = len(flows_perm) + len(flows)
        time = element[0]
        src = element[1]
        dst = element[2]
        nv_bytes = element[3]
        ...
        if( (src_host_flow_cnt[src] < fcnt) and (dst_host_flow_cnt[dst] < fcnt) ):
            next_time = time + nv_intarr.value()
            flows_perm.append([time, src, dst, flow_id, size])
            heapq.heappush(pq,[next_time, src, dst, nv_bytes, nv_intarr])
            src_host_flow_cnt[src] += 1
            dst_host_flow_cnt[dst] += 1
        else:
...
def write_to_file_homa(packet_size, header_size, output_file, flows):
    file = open(output_file, "w")
    # file.write(str(len(flows))+'\n')
    last_time = flows[0][0]
    for f in flows:
        s = ""
        s += "{0} {1} {2} {3} {4}\n".format(f[3], f[1], f[2], f[4], f[0] - last_time)
        last_time = f[0]
        file.write(s)
    file.close()
...
def main():
    ...
    parser.add_argument('-p','--packetsize',default=1500,
    ...
    parser.add_argument('-np','--numpackets',default=5,
    ...
    parser.add_argument('-m', '--meanflowsize', default=-1,
        help='mean flow size in KB')
    ...
    parser.add_argument('-r','--rtt', default='0.000005',
    ...
    parser.add_argument('-iat','--iat',default=0,
        help='type of iat distribution [0: exponentially sampled, 1: constant uniform value')
    parser.add_argument('-ov','--ov',default=1,
        help='oversubscription')
    ...
    mean_flow_size = int(args.meanflowsize)
    ...
    if(gen_type == 'P'):
        flows = poissonFlowGenerator(packet_size, header_size, mean_flow_size,num_flows, num_hosts, bandwidth, load, cdf, 1)
        ...
        flows = poissonFlowGeneratorMaxLimits(packet_size, header_size, mean_flow_size,num_flows, num_hosts, bandwidth, load, cdf, 1,fcnt)
        test_avg_load(num_hosts,flows,bandwidth,True,fcnt)
        write_to_file_dcqcn(packet_size, header_size, output, flows)
    elif(gen_type == 'PDR'):
        assert(num_flows <= (num_hosts * fcnt))
        flows = poissonFlowGeneratorMaxLimitsRandDest(packet_size, header_size, mean_flow_size,num_flows, num_hosts, bandwidth, load, cdf, 1,fcnt)
        test_avg_load(num_hosts,flows,bandwidth,True,fcnt)
        write_to_file_dcqcn(packet_size, header_size, output, flows)
    elif(gen_type == 'DI'):
        assert(num_flows <= (num_hosts * fcnt))
        ...
        flows = poissonSemiPermutationFlowGeneratorMaxLimits(packet_size, header_size, mean_flow_size,num_flows, num_hosts, bandwidth, load, cdf, 1,fcnt,alpha,iat)
        test_avg_load(num_hosts,flows,bandwidth,True,fcnt)
        write_to_file_homa(packet_size,header_size,output,flows)
    elif(gen_type == 'HOMA-OV'):
        assert(num_flows <= (num_hosts * fcnt))
        flows = poissonSemiPermutationFlowGeneratorMaxLimitsOversubscription(packet_size, header_size, mean_flow_size,num_flows, num_hosts, bandwidth, load, cdf, 1,fcnt,alpha,iat,ov,num_hosts_per_leaf)
        test_avg_load(num_hosts,flows,bandwidth,True,fcnt)
        write_to_file_homa(packet_size,header_size,output,flows)
    elif(gen_type == 'IC'):
        assert(num_flows <= (num_hosts * fcnt))
        ...
        test_avg_load(num_hosts,flows,bandwidth,False,fcnt)
        write_to_file_dcqcn(packet_size,header_size,output,flows)
    elif(gen_type == 'DISP'):
        assert(num_flows <= (num_hosts * fcnt))
        flows1,flows2 = dcqcnIncastSemiPermGenerator(packet_size,header_size,num_hosts_per_leaf,num_leaf,rtt, num_packets, fcnt, incast_degree,cdf,1,alpha,load,mean_flow_size,num_flows,bandwidth)
